...
# -------------------------- 5. Sample Size Gradient (Changed to percentages: 10%~90%) --------------------------
...
# ✅ Core update: Set percentages 10% ~ 90%
sample_percentages = [10, 20, 30, 40, 50, 60, 70, 80, 90]
...
print(f"Sample size percentages: {sample_percentages} (%)")
print("-" * 50)
...
# ✅ Iterate over percentages instead of multiples
for pct in sample_percentages:
    print(f"\n{'=' * 60}")
    print(f"Processing sample size percentage: {pct}%")
    print(f"{'=' * 60}")

    # Calculate target sample size: real IDP baseline × current percentage
    target_sample_size = int(real_base_sample_size * pct / 100)
    # Ensure not exceeding maximum available predicted IDP samples
    target_sample_size = min(target_sample_size, max_pred_sample_size)

    print(f"Target sample size: {target_sample_size} (baseline: {real_base_sample_size} × {pct}%)")
...
detailed_save_path = f"{seed_result_dir}/detailed_results_{data_type}_low_{seed_identifier}.csv"
detailed_results_df.to_csv(detailed_save_path, index=False, encoding='utf-8-sig')

simplified_save_path = f"{seed_result_dir}/simplified_auc_results_{data_type}_low_{seed_identifier}.csv"
simplified_results_df.to_csv(simplified_save_path, index=False, encoding='utf-8-sig')

# Group by percentage for summary
summary_df = simplified_results_df.groupby('sample_percentage').agg({
...
summary_save_path = f"{seed_result_dir}/summary_by_sample_percentage_{data_type}_low_{seed_identifier}.csv"
