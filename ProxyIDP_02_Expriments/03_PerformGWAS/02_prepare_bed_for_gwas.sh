...
INPUT_DIR="${BASE_DIR}/zhanghy/projects/MModal_IDPs_Pred/Gene/pgen_validation"          # Directory containing input pgen files
OUTPUT_DIR="${BASE_DIR}/zhanghy/projects/MModal_IDPs_Pred/Gene/bed_validation" # Output directory for results
...
for chr in {1..22}; do
    ...
    if [ -f "${input_prefix}.pgen" ] && [ -f "${input_prefix}.pvar" ] && [ -f "${input_prefix}.psam" ]; then
        # Run plink2 command to convert pgen to bed format
...
echo "All chromosomes converted to BED format successfully!"